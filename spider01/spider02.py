#!/usr/bin/env  python3
# -*- coding: utf-8 -*-
"""
 File Name：  spider02
 Author :  seven
 Change Activity:
	 2018/11/13:
"""

# 爬取代理 多线程验证代理可用性
import urllib.request
import urllib
import re
import time
import random
import socket
import threading

proxy_list = []
for page in range(2, 6):
	# url='http://ip84.com/dlgn/'+str(page)
	url = 'http://www.xicidaili.com/nn/' + str(page)  # 西刺代理
	headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; WOW64)"}
	request = urllib.request.Request(url=url, headers=headers)
	response = urllib.request.urlopen(request)
	content = response.read().decode('utf-8')
	print('get page', page)
	pattern = re.compile('<td>(\d.*?)</td>')  # 截取<td>与</td>之间第一个数为数字的内容
	ip_page = re.findall(pattern, str(content))
	proxy_list.extend(ip_page)
	time.sleep(random.choice(range(1, 3)))
# 打印抓取内容
print('代理IP地址', '\t', '\t', '端口', '\t', '存活时间', '\t', '验证时间')
for i in range(0, len(proxy_list), 4):
	print(proxy_list[i], '\t', '\t', proxy_list[i + 1], '\t', proxy_list[i + 2], '\t', proxy_list[i + 3])

# 整理代理IP格式
proxies = []
for i in range(0, len(proxy_list), 4):
	proxy_host = proxy_list[i] + ':' + proxy_list[i + 1]
	proxy_temp = {"http": proxy_host}
	proxies.append(proxy_temp)

proxy_ip = open('proxy_ip.txt', 'w')  # 新建一个储存有效IP的文档
lock = threading.Lock()  # 建立一个锁


def test_proxy(proxy):
	socket.setdefaulttimeout(5)  # 设置全局超时时间
	url = "https://www.haha.mx/pic"  # 打算爬取的网址
	try:
		proxy_support = urllib.request.ProxyHandler(proxies[proxy])
		opener = urllib.request.build_opener(proxy_support)
		opener.addheaders = [("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64)")]
		urllib.request.install_opener(opener)
		res = urllib.request.urlopen(url).read()
		lock.acquire()  # 获得锁
		print(proxies[proxy], 'is OK')
		proxy_ip.write('%s\n' % str(proxies[proxy]))  # 写入该代理IP
		lock.release()  # 释放锁
	except Exception as e:
		lock.acquire()
		print(proxies[proxy], e)
		lock.release()


# 单线程验证
# for i in range(len(proxies)):
#     test_proxy(i)

# 多线程验证    
threads = []
for i in range(len(proxies)):
	thread = threading.Thread(target=test_proxy, args=[i])
	threads.append(thread)
	thread.start()
# 阻塞主进程，等待所有子线程结束
for thread in threads:
	thread.join()

proxy_ip.close()  # 关闭文件
